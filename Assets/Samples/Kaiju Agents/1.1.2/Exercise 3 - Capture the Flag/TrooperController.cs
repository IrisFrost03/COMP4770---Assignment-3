...
namespace KaijuSolutions.Agents.Exercises.CTF
{
    ...
    public class TrooperController : KaijuController
    {
        ...
        private void OnSenseEnemies(TrooperEnemyVisionSensor sensor) {
            if (sensor.Observed.Count > 0 && trooper.CanAttack) {
                Transform nearestEnemy = Position.Nearest(sensor.Observed, out float distance);

                // Look at and attack the enemy
                Agent.LookTransform = nearestEnemy;
                trooper.Attack();
            }
            else {
                trooper.StopAttacking();
            }
        }
